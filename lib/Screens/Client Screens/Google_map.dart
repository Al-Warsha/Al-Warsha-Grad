...
class _Google_mapState extends State<Google_map> {
  ...
  Widget build(BuildContext context) {
    bool emergency = widget
        .emergency; // Access the 'emergency' variable using 'widget'
    return Scaffold(
      body: GoogleMap(
        ...
        initialCameraPosition: mycurrentlocation == null
            ? _kGooglePlex
            : mycurrentlocation!,
        markers: markers,
        ...
        onPressed: () async {
          if (emergency) {
            Navigator.pop(
              ...
              MaterialPageRoute(
                builder: (context) =>
                    EmergencyRoadHelp(mechanicId: businessOwnerId!,
  ...
  {
    permissionStatus =
    await location.hasPermission(); // low hya true hyd5ol 3ala else 3ala tol
    if (permissionStatus ==
        PermissionStatus.denied) //bya5od permission mn user tany
        {
      permissionStatus = await location.requestPermission();
      return permissionStatus == PermissionStatus
          .granted; // low hya granted hy-return true, low l2 yb2a return false
    } else {
  ...
  void updateUserLocation() async {
    if (mounted) {
      mycurrentlocation = CameraPosition(
          ...
          // dah m3nah eno m4 hygelo ba null
      ...
      final GoogleMapController controller = await _controller
          .future; // elsatr dah w ely b3do bywdony 3ala current location 3ala tol,
      controller.animateCamera(CameraUpdate.newCameraPosition(
          mycurrentlocation!)); // bs low location m4 mfto7 3ndk hywdek la defult
      setState(() {
...
//api key
//AIzaSyDRy7stM8lVq1LbYPHNqdgAoGkdleWS7